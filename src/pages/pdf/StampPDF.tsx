  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        if (Math.abs(width - previewWidth) > 5) { // Debounce small changes
  ...
  const PDFLivePreview = memo(() => {
    ...
    return (
      <div
        ...
        className="relative w-full h-full min-h-[600px] bg-white overflow-hidden rounded-lg shadow-inner"
      >
        <Document
          file={memoizedPdfFile}
          onLoadSuccess={() => { setPdfLoading(false); setPdfError(null); }}
          onLoadError={(err) => { setPdfLoading(false); setPdfError(err.message); }}
          loading={null}
          key={memoizedPdfFile?.name} // Stable key to prevent remount
        >
          <Page
            pageNumber={1}
            ...
            <div
              style={{
                ...
                fontFamily:
                  ...
                  fontFamily === "Times" ? "serif" : "monospace",
                color: textColor,
                opacity: opacity / 100,
                fontWeight: "bold",
                whiteSpace: "nowrap",
    ...
    <div className="max-w-3xl mx-auto">
      <Card>
        <CardContent className="p-12 text-center">
          ...
          <h2 className="text-2xl font-bold mb-3">Add Watermark to PDF</h2>
          <p className="text-muted-foreground mb-8">Upload your PDF and customize the watermark</p> */}
